...
beforeEach(async () => {
  ...
  aLinkTokenAddressContract = new web3.eth.Contract(
    tokenABI,
    aLinkTokenAddress
  );
  ...
  // Create a new pool.
  await factoryV2.methods
    .createPool("LINK", "TEST", 1000, newAccountData.address)
    .send({ from: admin, gas: 10 ** 7 });

  console.log(
    "Pool exists: ",
    await factoryV2.methods.verifyPool("TEST").call()
  );
  ...
  await factoryV2.methods
    .verifyPoolAccess("TEST", signObject.messageHash, signObject.signature)
    .send({ from: user });

  await factoryV2.methods
    .getVerifiedStatus("TEST")
    .call({ from: user })
    .then((result) => {
...
describe("Deposit process", async function () {
  it("Should be able to create pools", async () => {
    ...
    // Create a new pool.
    await factoryV2.methods
      .createPool(linkTokenAddress, "TEST", 50, newAccountData.address)
      .send({ from: admin });
    console.log(
      "Pool exists: ",
      await factoryV2.methods.verifyPool("TEST").call()
  ...
  it.only("Should allow users to deposit", async () => {
    ...
    _aToken = new web3.eth.Contract(
      compiledIScaledBalanceToken.abi,
      aLinkTokenAddress
    );

    // Scaled balance
    await _aToken.methods
      .scaledBalanceOf(factoryV2.options.address)
      .call()
      .then((a) => {
        console.log("Scaled balance: ", a);
      });

    let userScaled;
    await factoryV2.methods
      .getUserScaledDeposit("TEST")
      .call({ from: user })
      .then((tx) => {
    ...
    await factoryV2.methods
      ...
      .then((tx) => {
        ...
        console.log(
          "aToken amount to be withdrawn (LI): ",
          (tx[0] * userScaled) / 10 ** 27
        );
        console.log(
          "aToken amount to be withdrawn (RNI): ",
          (tx[1] * userScaled) / 10 ** 27
        );
      });
    ...
    await _aToken.methods
      ...
      .call()
      .then((a) => {
        console.log("Scaled balance after withdrawal: ", a);
      });
